...
export default function GroupCard({ group, variant }: GroupCardProps) {
  ...
  return (
    <div className={`relative overflow-hidden block ${cardStyles}`}>
      {isCurrent ? null : (
        <div className="cursor-default absolute w-65 flex flex-col items-center justify-end z-10 pb-4 min-h-20 py-6 -top-4 -right-27 rotate-45 bg-blue-600 dark:bg-slate-900 text-xs font-semibold text-gray-100 dark:text-gray-100 border border-gray-300 dark:border-gray-600 rounded shadow-md">          <span className="italic">Concluído</span>
        </div>
      )}
      <div className="mb-2">
